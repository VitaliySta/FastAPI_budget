...
class OperationService:
    ...
    def _get(self, user_id: int, operation_id: int):
        operation = (
            self.session
        ...
        if not operation:
            raise HTTPException(
                ...
                detail=f'Оперции с id = {operation_id} не существует!'
            )
        return operation
    ...
    def create_many(self, user_id: int, operations_data: List[OperationCreate]):
        operations = [
            tables.Operation(
                **operation_data.dict(),
                user_id=user_id,
    ...
    def create(self, user_id: int, operation_date: OperationCreate):
    ...
    def update(self, user_id: int, operation_id: int, operation_data: OperationUpdate):
        ...
        for fiield, value in operation_data:
            setattr(operation, fiield, value)
        self.session.commit()
        return operation
